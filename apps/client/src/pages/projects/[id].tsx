...
import type { Project, Team, User } from "@packages/shared";
...
export default function ProjectDetailsPage() {
  ...
  // Buscar detalhes do projeto e times associados
  const fetchData = async () => {
    setLoading(true);
    try {
      const [projectRes, teamsRes, allTeamsRes] = await Promise.all([
  ...
  const handleRemoveTeam = async (teamId: string) => {
    ...
    if (res.ok && res.data.success) {
      ...
      addToast({
        ...
        description: res.data?.message || res.data?.error || "Erro ao remover time do projeto.",
        type: "error",
      });
    }
